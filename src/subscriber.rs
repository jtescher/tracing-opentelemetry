...
impl<S: api::Span> field::Visit for SpanAttributeVisitor<S> {
    fn record_str(&mut self, field: &field::Field, value: &str) {
        let _ = self
            .0
            .try_lock()
            .map(|mut span| span.set_attribute(field.name().to_string(), value.to_string()));
    }
    fn record_debug(&mut self, field: &field::Field, value: &dyn fmt::Debug) {
        let _ = self
            .0
            .try_lock()
            .map(|mut span| span.set_attribute(field.name().to_string(), format!("{:?}", value)));
    }
}
...
impl<T: api::Tracer + 'static> Subscriber for OpentelemetrySubscriber<T> {
    ...
    fn exit(&self, id: &Id) {
        ...
        self.tracer.drop_span(id.into_u64());
    }
}
